...
public interface OrderItemRepository extends JpaRepository<OrderItem, Integer> {
    ...
    @Query(value = "select oi from OrderItem oi join fetch oi.item where oi.status = 1 and oi.item.item.category = 1",
            countQuery = "select count(oi) from OrderItem oi where oi.status = 1 and oi.item.item.category = 1")
    Page<OrderItem> findAllDrinksInPreparation(Pageable pageable);

    @Query(value = "select oi from OrderItem oi join fetch oi.item where oi.status = 1 and oi.item.item.category = 0",
            countQuery = "select count(oi) from OrderItem oi where oi.status = 1 and oi.item.item.category = 0")
